...
num_steps = 500  # 总迭代步数
batch_size = 128  # 每迭代一步，传入的训练集样本的个数
display_step = 100  # 每多少步迭代打印一次loss和预测结果
...
loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
    logits=logits, labels=Y))  # 用softmax_cross_entropy_with_logits做多分类？？？
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
train_op = optimizer.minimize(loss_op)

# Evaluate model (with test logits, for dropout to be disabled)，定义检验模型效果的方式
correct_pred = tf.equal(tf.argmax(logits, 1), tf.argmax(Y, 1))
print("tf.argmax(logits, 1)={}".format(tf.argmax(logits, 1)))
...
with tf.Session() as sess:
    ...
    for step in range(1, num_steps + 1):
        ...
        if step % display_step == 0 or step == 1:
            ...
            # print("batch_y[0]={}".format(batch_y[0]))
